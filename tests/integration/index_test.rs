...
async fn test_create_and_drop_index() {
    let store = KvStore::<super::Backend>::at_location(Location::InMemory).unwrap();
...
async fn test_unique_index_constraint() {
    ...
    index_manager
        .update_indexes("user:1", None, "test@example.com")
        .unwrap();
...
async fn test_index_manager_operations() {
    ...
    if let Some(age_idx) = index_manager.get_index("age") {
        ...
        // Range query
        let keys = age_idx
            .find_range(&IndexValue::Integer(28), &IndexValue::Integer(36))
            .unwrap();
