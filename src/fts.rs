...
impl FtsIndex {
    ...
    pub fn index_document(&self, doc_id: String, content: &str) -> Result<(), IndexError> {
        ...
        let mut inverted_index =
            self.inverted_index
                .write()
                .map_err(|_| IndexError::OperationNotSupported {
            ...
            if let Some(docs) = inverted_index.get(term) {
                ...
                for doc_info in docs {
                    ...
                    let score = idf * (tf * (k1 + 1.0))
                        / (tf + k1.mul_add(b.mul_add(normalized_len, 1.0 - b), 0.0));
    ...
    pub fn phrase_search(&self, phrase: &str, limit: usize) -> Vec<SearchResult> {
        ...
        let Some(candidates) = candidate_docs else {
            return Vec::new();
        };
...
impl FtsManager {
    ...
    pub fn create_index_with_tokenizer(
        ...
        indexes.insert(name.clone(), FtsIndex::with_tokenizer(name, tokenizer));
        drop(indexes);
        Ok(())
    }
